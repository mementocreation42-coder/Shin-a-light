...
/** 第1段階：パスワード */
function PasswordStep({ from, devPassword, onAdvance }: { from: string; devPassword?: string; onAdvance: (s: LoginState) => void }) {
  ...
  return (
    <form action={formAction}>
      ...
      <div style={{ marginBottom: '16px' }}>
        ...
        <input name="password" type="password" autoComplete="current-password" required autoFocus defaultValue={devPassword} placeholder="••••••••" style={inputStyle} />
      </div>
...
function LoginForm({ devPassword }: { devPassword?: string }) {
    ...
    <div style={{
      ...
      <div style={{
        ...
        {stage === 'totp'
          ...
          : <PasswordStep from={from} devPassword={devPassword} onAdvance={() => setStage('totp')} />}
      </div>
    </div>
  );
}
...
/**
 * @param devPassword 開発環境でのみ渡される。パスワード欄に初期入力しておき、毎回打たずに済ませる
 */
export default function LoginClient({ devPassword }: { devPassword?: string }) {
  return (
    <Suspense>
      <LoginForm devPassword={devPassword} />
    </Suspense>
  );
}
